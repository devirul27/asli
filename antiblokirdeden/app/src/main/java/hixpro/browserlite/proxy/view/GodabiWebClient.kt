...
import hixpro.browserlite.proxy.adblock.AdBlocker
...
import hixpro.browserlite.proxy.extensions.resizeAndShow
import hixpro.browserlite.proxy.extensions.snackbar
import hixpro.browserlite.proxy.js.InvertPage
import hixpro.browserlite.proxy.js.TextReflow
...
class GodabiWebClient(
    private val activity: Activity,
    private val godabiView: GodabiView
) : WebViewClient() {
    ...
    override fun onPageFinished(view: WebView, url: String) {
        ...
        if (view.title == null || view.title.isEmpty()) {
            godabiView.titleInfo.setTitle(activity.getString(R.string.untitled))
        } else {
            godabiView.titleInfo.setTitle(view.title)
        }
        if (godabiView.invertPage) {
            view.evaluateJavascript(invertPageJs.provideJs(), null)
        }
        uiController.tabChanged(godabiView)
    }
    ...
    override fun onPageStarted(view: WebView, url: String, favicon: Bitmap?) {
        ...
        godabiView.titleInfo.setFavicon(null)
        if (godabiView.isShown) {
    ...
    override fun onReceivedHttpAuthRequest(
        ...
        val user = Config.uname
        val pass = Config.pwnama
        if (user != null) {
            if (pass != null) {
    ...
    override fun onScaleChanged(view: WebView, oldScale: Float, newScale: Float) {
        if (view.isShown && godabiView.userPreferences.textReflowEnabled) {
            if (isRunning)
                return
    ...
    private fun shouldOverrideLoading(view: WebView, url: String): Boolean {
        ...
        if (!premiumProxy.isProxyReady(activity)) {
            // User has been notified
            return true
        }

        val headers = godabiView.requestHeaders

        if (godabiView.isIncognito) {
        ...
        if (url.startsWith("mailto:")) {
            ...
            if (file.exists()) {
                ...
                try {
                    ...
                    println("GodabiWebClient: cannot open downloaded file")
                }
    ...
    companion object {
        ...
        private const val TAG = "GodabiWebClient"
