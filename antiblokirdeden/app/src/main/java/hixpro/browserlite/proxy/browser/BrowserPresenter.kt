...
import hixpro.browserlite.proxy.constant.INTENT_ORIGIN
...
import hixpro.browserlite.proxy.view.BundleInitializer
import hixpro.browserlite.proxy.view.GodabiView
import hixpro.browserlite.proxy.view.TabInitializer
import hixpro.browserlite.proxy.view.UrlInitializer
...
class BrowserPresenter(
    ...
    private var currentTab: GodabiView? = null
     ...
     */
    fun tabChangeOccurred(tab: GodabiView?) = tab?.let {
        view.notifyTabViewChanged(tabsModel.indexOfTab(it))
    }

    private fun onTabChanged(newTab: GodabiView?) {
