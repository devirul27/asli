...
(function () {
    ...
    window.Framework7 = function (params) {
        ...
        app.editLink = function (text, title, callbackOk, callbackCancel, array) {
            if (typeof title === 'function') {
                callbackCancel = arguments[2];
                callbackOk = arguments[1];
                title = undefined;
            ...
            var url = array.url;
            var name = array.name;
            if(url !== '' && url !== null) urls = url;
            if(name !== '' && name !== null) names = name;
            return app.modal({
                text: text || '',
                title: typeof title === 'undefined' ? app.params.modalTitle : title,
                afterText: '' +
                '<div class="input-field"><input type="text" name="name" placeholder="Title" value="'+names+'" class="modal-text-input"></div>' +
                '<div class="input-field"><input type="text" name="url" placeholder="Link" value="'+urls+'" class="modal-text-input"></div>' +
                '',
                buttons: [
                    {
        ...
        app.weatherModal = function (text, title, callbackOk, callbackCancel, city) {
            if (typeof title === 'function') {
                callbackCancel = arguments[2];
                callbackOk = arguments[1];
                title = undefined;
            ...
            return app.modal({
                text: text || '',
                title: typeof title === 'undefined' ? app.params.modalTitle : title,
                afterText: '<div class="input-field"><input type="text" value="'+city+'" class="modal-text-input"></div>',
                buttons: [
                    {
                        text: app.params.modalButtonCancel
                    },
        ...
        app.addLink = function (text, title, callbackOk, callbackCancel, url_add, title_add) {
            if (typeof title === 'function') {
                callbackCancel = arguments[2];
                callbackOk = arguments[1];
                title = undefined;
            }
            var url = (url_add != '' && url_add != null) ? url_add : '';
            var title_link = (title_add != '' && title_add != null) ? title_add : '';
            return app.modal({
                text: text || '',
                title: typeof title === 'undefined' ? app.params.modalTitle : title,
                afterText: '<div class="input-field"><input type="text" placeholder="title" value="'+title_link+'"  name="title" class="modal-text-input"></div>' +
                '<div class="input-field"><input type="text" placeholder="url" name="url" id="url_edit" onblur="if(this.value == \'http://\'){this.value = \'\';}" onfocus="this.value=\'http://\'" value="'+url+'" class="modal-text-input"></div>',
                buttons: [
                    {
                        text: app.params.modalButtonCancel
                    },
                ...
                onClick: function (modal, index) {
                    var title = $(modal).find('.modal-text-input[name="title"]').val();
                    var url = $(modal).find('.modal-text-input[name="url"]').val();
                    if (index === 0 && callbackCancel) callbackCancel(title, url);
                    if (index === 1 && callbackOk) callbackOk(title, url);
                }
            });
        };
    ...
    var Dom7 = (function () {
        ...
        $.ajax = function (options) {
            ...
            var xhr;
            if (options.url.indexOf('pixabay') + 1) {
                // Create XHR
                xhr = new XMLHttpRequest();
            }else {
                if(options.url.indexOf('nytimes') + 1){
                    xhr = new XMLHttpRequest();
                } else {
                    if('AppsgeyserJSInterface' in window || 'NotificationInterface' in window){
                        xhr = new appsgeyser.web.XmlHttpRequest();
                    } else {
                        // Create XHR
                        xhr = new XMLHttpRequest();
                    }
