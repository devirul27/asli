...
public class NetworkSpace {
    ...
    public Collection<ipAddress> getNetworks(boolean included) {
        ...
        for (ipAddress ip : mIpAddresses) {
            if (ip.included == included) ips.add(ip);
        }
        return ips;
    }
        ...
        while (currentNet != null) {
            ...
            if (nextNet == null || currentNet.getLastAddress().compareTo(nextNet.getFirstAddress()) == -1) {
                ...
                if (currentNet.getFirstAddress().equals(nextNet.getFirstAddress()) && currentNet.networkMask >= nextNet.networkMask) {
                    if (currentNet.included == nextNet.included) {
                        ...
                        if (newNets[0].getLastAddress().equals(currentNet.getLastAddress())) {
                            if (BuildConfig.DEBUG) {
                                Log.i(TAG, "generateIPList: "+newNets[0].networkMask+", "+currentNet.networkMask);
                            }
                    ...
                    if (BuildConfig.DEBUG) {
                        Log.i(TAG, "generateIPList: "+(currentNet.networkMask < nextNet.networkMask));
                        Log.i(TAG, "generateIPList: "+(nextNet.getFirstAddress().compareTo(currentNet.getFirstAddress()) == 1));
                        Log.i(TAG, "generateIPList: "+(currentNet.getLastAddress().compareTo(nextNet.getLastAddress()) != -1));
                    }
                    ...
                    if (currentNet.included == nextNet.included) {
                        ...
                        if (newNets[1].networkMask == nextNet.networkMask) {
                            if (BuildConfig.DEBUG) {
                                Log.i(TAG, "generateIPList: "+newNets[1].getFirstAddress().equals(nextNet.getFirstAddress()));
                                Log.i(TAG, "generateIPList: "+newNets[1].getLastAddress().equals(currentNet.getLastAddress()));
    ...
    static class ipAddress implements Comparable<ipAddress> {
        ...
        public int compareTo(@NonNull ipAddress another) {
            ...
            if (networkMask > another.networkMask) return -1;
            else if (another.networkMask == networkMask) return 0;
            else return 1;
        }
        ...
        public boolean equals(Object o) {
            if (!(o instanceof ipAddress)) return super.equals(o);
        ...
        public BigInteger getLastAddress() {
            if (lastAddress == null) lastAddress = getMaskedAddress(true);
            return lastAddress;
        }

        public BigInteger getFirstAddress() {
            if (firstAddress == null) firstAddress = getMaskedAddress(false);
            return firstAddress;
        }
        ...
        public String toString() {
            //String in = included ? "+" : "-";
            if (isV4) return String.format(Locale.US, "%s/%d", getIPv4Address(), networkMask);
            else return String.format(Locale.US, "%s/%d", getIPv6Address(), networkMask);
        }
        ...
        public ipAddress[] split() {
            ...
            if (BuildConfig.DEBUG) {
                Log.i(TAG, "generateIPList: "+secondHalf.getLastAddress().equals(getLastAddress()));
            }
        ...
        String getIPv4Address() {
            if (BuildConfig.DEBUG) {
                Log.i(TAG, "generateIPList: "+isV4);
                Log.i(TAG, "generateIPList: "+(netAddress.longValue() <= 0xffffffffl));
                Log.i(TAG, "generateIPList: "+(netAddress.longValue() >= 0));
            }
